if(!"librarian" %in% rownames(installed.packages())) {
  install.packages("librarian", repos = "https://cran.posit.com/", dep = TRUE)
}
      ...
      tabItem(tabName = "tab1",
              ...
              fluidRow( # ---- Choix des variables
                ...
                box(
                  ...
                  actionButton("load_dataset", "Load/Reload datasets") ,
                  uiOutput("choix_var") ),
                
                box( tableOutput("liste_var") )
...
server = function(input, output, session) {
  ...
  board_dataexplorer <- reactive({
    board <- board_folder( path = paste0( parseQueryString(session$clientData$url_search)[["param"]], "/board_dataexplorer") )
    return(board)
  })
  
  base_ref <- eventReactive(input$load_dataset, {
    aux <- switch(input$database,
                  # "Ma base" = database(), # NB : avant, database passer comme argument de fonction, car l'appli shiny était ausein d'une fonction
                  "Ma base" = pin_read(board_dataexplorer(), "database"), 
                  "Jeu Orange" = Orange,
                  "Jeu iris" = iris,
                  "Jeu mtcars" = mtcars,
                  "Jeu swiss" = swiss)
